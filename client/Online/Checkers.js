...
socket.emit("join");  // joining room


socket.on("setTurn", size => {
   if (size == 1)
   {
      mySide = true
      stopMove()
      ...
      document.querySelectorAll(".pic").forEach((pic) => {pic.style.transform =  "rotate(180deg)";})
   }
})
...
socket.on("enemyMove", (id, activeCoinId, queen) => {
   activeQueen = queen;
   console.log(queen)
   activeCoin = document.getElementById(activeCoinId).getElementsByTagName("img")[0]; 
   moveCoin(id);
...
function checkForKill(){
   ...
   if (turn == true){
      ...
      console.log(checkedCoins.length);
...
function showOnlyMove(){
   ...
   coins.forEach(coin => {
      ...
      if (coin.src.includes("black") || coin.src.includes("Black") || coin.src.includes("white") || coin.src.includes("White")){
         console.log(checkedCoins, "mrdly")
...
function moveCoin(go){
   var go2;
   if (go > 0 && go < 50){       // move from server
      var img = document.getElementById(go).getElementsByTagName("img")[0];
      go2 = go;
   }else{
      var img = document.getElementById(go.parentNode.id).getElementsByTagName("img")[0];
      go2 = go.parentNode.id;
   }
   ...
   if (turn == true && isQueen(go2)){                          // QUEEN
      
      img.src = "Pics/whiteQueen.png";
   }
   if (turn == false && isQueen(go2)){
      img.src = "Pics/BlackQueen.png";
   }
   activeCoin.src = "";
   console.log(activeQueen)
   if (go2 != go){
      socket.emit("enemyMove", go2, activeCoin.parentNode.id, activeQueen)
   }
...
function killEnemy(dead, move, target){
  ...
  if (dead.length > 0 && move.length > 0){
     if (move[0] == target2){
      ...
      socket.emit("killedPiece", dead[0].id)
      console.log(img);
      img.src = "";
      killed = true;
     }
     if (move[1] == target2){
      ...
      socket.emit("killedPiece", dead[1].id)
      console.log(img);
      img.src = "";
      killed = true;
     }
     if (move[2] == target2){
      ...
      socket.emit("killedPiece", dead[2].id)
      console.log(img);
      img.src = "";
      killed = true;
     }
     if (move[3] == target2){
      ...
      socket.emit("killedPiece", dead[3].id)
      console.log(img);
      img.src = "";
      killed = true;
     }
